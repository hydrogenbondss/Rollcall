...
function Toast({ message, visible }: { message: string; visible: boolean }) {
 return (
 <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[300] transition-all duration-300 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
...
export default function ProductDetail() {
 ...
 const sameCountry = products.filter(p => p.country === product.country && p.id !== product.id && p.brand !== product.brand)
 ...
 if (leftRef.current) gsap.from(leftRef.current, { opacity: 0, x: -20, duration: 0.7, ease: 'power3.out' })
 if (rightRef.current) {
 gsap.from(rightRef.current.querySelectorAll('.detail-item'), { opacity: 0, x: 20, duration: 0.5, stagger: 0.06, ease: 'power3.out', delay: 0.2 })
 }
 if (relatedRef.current) {
 gsap.from(relatedRef.current.querySelectorAll('.related-card'), { opacity: 0, y: 20, duration: 0.5, stagger: 0.06, ease: 'power3.out',
 ...
 else { addToCompare(product); setIsOpen(true) }
 }
 ...
 { icon: MapPin, label: 'Made in', value: `${product.manufacturedIn} (${product.manufacturer})` },
 { icon: Globe, label: 'Available', value: product.availableIn.join(', ') },
 ]

 return (
 <div className="min-h-screen bg-[#0d0d0d]">
 <Navigation /><Toast {...toast} />
 ...
 <div className="grid lg:grid-cols-[55%_45%] gap-10 lg:gap-16">
 {/* Image */}
 <div ref={leftRef}>
 ...
 'Thickest': 'bg-[#c4728e]/10 text-[#c4728e] border-[#c4728e]/20',
 'Most Luxurious': 'bg-[#c28223]/10 text-[#c28223] border-[#c28223]/20',
 'Most Rolls': 'bg-[#228b68]/10 text-[#228b68] border-[#228b68]/20',
 ...
 <span key={h} className="inline-flex items-center gap-1.5 bg-white/5 rounded-lg px-3 py-2 font-body text-[12px] text-[#999]">
 <Hotel className="w-3 h-3 text-[#c4bdb5]" />{h}
 </span>
 ))}
 </div>
 </div>
 ...
 <button onClick={handleCompareToggle} className={`flex-1 flex items-center justify-center gap-1.5 font-body text-sm px-6 py-3.5 rounded-full border transition-colors ${
 comparing ? 'bg-[#f0ece8] text-[#0d0d0d] border-[#f0ece8]' : 'bg-transparent text-[#888] border-white/10 hover:border-white/30'
 }`}>
 ...
 <div ref={relatedRef} className="border-t border-white/5 bg-[#0d0d0d] pt-16 pb-20">
 ...
 <div key={rp.id} className="related-card"><ProductCard product={rp} index={i} isVisible={true} /></div>
 ))}
 </div>
 </div>
 </div>
 ...
 {/* Mobile sticky */}
 <div className={`fixed bottom-0 left-0 right-0 z-50 bg-[#141414] border-t border-white/5 px-4 py-3 md:hidden transition-transform duration-300 ${showMobileBar ? 'translate-y-0' : 'translate-y-full'}`}>
 <div className="flex items-center gap-3">
 <button onClick={handleCompareToggle} className={`flex items-center justify-center gap-1.5 font-body text-sm px-4 py-3 rounded-full border transition-colors ${
 comparing ? 'bg-[#f0ece8] text-[#0d0d0d] border-[#f0ece8]' : 'bg-transparent text-[#888] border-white/5'
 }`}>
 {comparing ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
 </button>
 <button onClick={handleShare} className="flex-1 flex items-center justify-center gap-1.5 bg-[#f0ece8] text-[#0d0d0d] font-body text-sm py-3 rounded-full">
 <Share2 className="w-4 h-4" strokeWidth={1.5} />Share Roll
 </button>
 </div>
 </div>
 </div>
