      ...
      <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${
        ...
        <div className="max-w-[1200px] mx-auto w-full px-6 sm:px-8 flex items-center justify-between h-16">
          <Link to="/" className="flex items-baseline gap-3 group">
            ...
            <span className="hidden sm:inline font-mono text-[9px] tracking-[0.2em] text-[#b6b0a6] uppercase group-hover:text-[#b6b0a6] transition-colors">Material Culture Archive</span>
          </Link>
            ...
            {allLinks.map((link) => {
              ...
              return (
                <Link key={link.to} to={link.to} className="group relative py-1">
                  <span className={`font-body text-[13px] transition-colors ${isActive ? 'text-[#f0ece8]' : 'text-[#b6b0a6] group-hover:text-[#f0ece8]'}`}>{link.label}</span>
          ...
          <div className="flex items-center gap-1">
            <button onClick={handleRandomRoll} className="hidden sm:flex items-center p-2 rounded-lg text-[#b6b0a6] hover:text-[#f0ece8] hover:bg-white/5 transition-all" title="Random Roll" aria-label="Random Roll">
              <Dices className="w-4 h-4" />
            </button>
            <button onClick={() => setMenuOpen(true)} className="md:hidden p-2 rounded-lg text-[#b6b0a6] hover:text-[#f0ece8] hover:bg-white/5 transition-all" aria-label="Open menu">
