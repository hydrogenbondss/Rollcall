...
export default function Stories() {
  ...
  const renderWords = (text: string) => {
    return text.split(' ').map((word, i) => (
      <span key={i} className="word inline" style={{ opacity: 0.92 }}>
        {word}{' '}
      </span>
    ))
  }
    ...
    <section ref={sectionRef} id="stories" className="w-full bg-[#0d0d0d] text-white relative overflow-hidden">
      ...
      <div className="max-w-[720px] mx-auto px-6 sm:px-8 pt-32 pb-20">
        <div className="essay-header">
          <div className="flex items-center gap-4 mb-8">
            <p className="font-mono text-[10px] uppercase tracking-[0.5em] text-white/70">Wall Text</p>
            <span className="w-8 h-px bg-white/30" />
            <p className="font-mono text-[10px] text-white/70">One Region</p>
          </div>
          ...
          <p className="font-serif-display text-xl sm:text-2xl italic text-white/80 leading-relaxed mb-20 max-w-lg">
      ...
      <div className="max-w-[720px] mx-auto px-6 sm:px-8 pb-32 space-y-24">
        ...
        <div className="essay-cite pt-8 border-t border-white/10">
          <p className="font-mono text-[10px] text-white/65 leading-relaxed">
