      ...
      <div className="max-w-[1200px] mx-auto px-6 sm:px-8">
        <div className="viz-title mb-14">
          <div className="flex items-center gap-3 mb-3">
            <BarChart3 className="w-4 h-4 text-[#b6b0a6]" strokeWidth={1.5} />
            <p className="font-body text-[10px] uppercase tracking-[0.4em] text-[#b6b0a6]">Data</p>
          </div>
          ...
          <div className="flex items-center gap-2 mt-6">
            <button onClick={() => setViewMode('price')} className={`px-4 py-2 rounded-full text-[11px] font-mono uppercase tracking-wider transition-all ${viewMode === 'price' ? 'bg-[#c28223]/15 text-[#c28223] border border-[#c28223]/20' : 'bg-white/5 text-[#b6b0a6] border border-white/5 hover:border-white/15'}`}>Price View</button>
            <button onClick={() => setViewMode('gdp')} className={`px-4 py-2 rounded-full text-[11px] font-mono uppercase tracking-wider transition-all ${viewMode === 'gdp' ? 'bg-[#c28223]/15 text-[#c28223] border border-[#c28223]/20' : 'bg-white/5 text-[#b6b0a6] border border-white/5 hover:border-white/15'}`}>GDP View</button>
          </div>
        </div>
            ...
            {tooltip && (
              <div className="absolute z-10 pointer-events-none" style={{
                ...
                <div className="bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 shadow-2xl whitespace-nowrap">
                  ...
                  <p className="font-body text-[11px] text-[#b6b0a6]">{tooltip.product.country} · {tooltip.product.ply}-ply</p>
                  <p className="font-body text-[11px] text-[#b6b0a6] mt-0.5">HK${(tooltip.product.priceUSD * hkdRate).toFixed(2)}</p>
                  {tooltip.gdp && (
            ...
            <div className="flex items-center justify-center gap-6 mt-2">
              {Object.entries(regionColors).map(([region, color]) => (
                <div key={region} className="flex items-center gap-2">
                  ...
                  <span className="font-body text-[11px] text-[#b6b0a6]">{region}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            {Object.entries(regionStats).map(([region, stats]) => (
              <div key={region} className="bg-[#141414] rounded-xl border border-white/5 p-5 hover:border-white/10 transition-all">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><p className="font-display text-xl text-[#f0ece8]">{stats.count}</p><p className="font-body text-[10px] text-[#b6b0a6] uppercase tracking-wider">Products</p></div>
                  <div><p className="font-display text-xl text-[#f0ece8]">{stats.avgPly}</p><p className="font-body text-[10px] text-[#b6b0a6] uppercase tracking-wider">Avg Ply</p></div>
                  {viewMode === 'gdp' && (
                    <div className="col-span-2"><p className="font-body text-[11px] text-[#b6b0a6]">Avg GDP: <span className="text-[#f0ece8]">${stats.avgGdp.toLocaleString()}</span></p></div>
                  )}
                </div>
              </div>
            ))}
