      ...
      <div className="max-w-[1200px] mx-auto px-6 sm:px-8">
        ...
        <div className="comm-cities mb-16 pt-8 border-t border-white/[0.04]">
          <div className="grid sm:grid-cols-3 gap-8 mb-12">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-wider text-[#555] mb-2">Current Status</p>
              <p className="font-body text-[13px] text-[#8a8279] leading-relaxed">
                43 specimens catalogued. 21 countries covered. The project launched in Hong Kong in early 2026.
              </p>
            </div>
            <div>
              <p className="font-mono text-[10px] uppercase tracking-wider text-[#555] mb-2">What We Need</p>
              <p className="font-body text-[13px] text-[#8a8279] leading-relaxed">
                First-hand photographs from Cambodia, Laos, Myanmar, and rural China. Verified product packaging from emerging markets.
              </p>
            </div>
            <div>
              <p className="font-mono text-[10px] uppercase tracking-wider text-[#555] mb-2">What We Offer</p>
              <p className="font-body text-[13px] text-[#8a8279] leading-relaxed">
          ...
          <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#555] mb-3">Cities Represented</p>
          <div className="flex flex-wrap gap-2">
            {['Hong Kong', 'Tokyo', 'Singapore', 'Bangkok', 'Mumbai', 'Dhaka', 'Jakarta', 'Manila', 'Seoul', 'Taipei', 'Yangon', 'Kathmandu', 'Hanoi', 'Phnom Penh', 'Colombo', 'Karachi', 'Lahore', 'Kuala Lumpur', 'Beijing', 'Ulaanbaatar', 'Vientiane'].map((city) => (
              <span key={city} className="font-body text-[11px] text-[#666] bg-white/[0.03] px-3 py-1 rounded-full">{city}</span>
            ))}
          </div>
        </div>
        ...
        <div className="comm-grid grid sm:grid-cols-2 gap-6">
          {engagementModes.map((mode) => (
            <div key={mode.title} className="comm-card group bg-[#141414] border border-white/[0.04] rounded-xl p-6 sm:p-8 hover:border-white/[0.08] transition-colors">
              ...
              <p className="font-body text-[13px] text-[#8a8279] leading-relaxed mb-6">{mode.description}</p>
              <a
                href={mode.href}
        ...
        <div className="mt-16 pt-16 border-t border-white/[0.04] text-center">
          <p className="font-body text-[15px] text-[#8a8279] leading-relaxed max-w-lg mx-auto mb-8">
