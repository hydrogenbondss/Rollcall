      ...
      <div className="max-w-[860px] mx-auto px-6 sm:px-8">
        <div className="map-title mb-8">
          <div className="flex items-center gap-3 mb-3">
            <MapPin className="w-4 h-4 text-[#b6b0a6]" strokeWidth={1.5} />
            <p className="font-body text-[10px] uppercase tracking-[0.4em] text-[#b6b0a6]">Coverage · supporting reference</p>
          </div>
          <h2 className="font-display text-3xl sm:text-4xl text-[#f0ece8] tracking-tight leading-[1.05]">Where the archive reaches</h2>
          <p className="font-body text-[13px] text-[#b6b0a6] mt-3 max-w-md">
            ...
            return (
              <div className="absolute z-10 pointer-events-none bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 shadow-2xl"
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-mono text-[9px] text-[#b6b0a6] uppercase">{d.region}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-mono text-[10px] text-[#b6b0a6]"><strong className="text-[#f0ece8]">{d.count}</strong> specimen{d.count > 1 ? 's' : ''}</span>
                  <span className="font-mono text-[10px] text-[#b6b0a6]">Avg <strong className="text-[#f0ece8]">{d.avgPly}</strong>-ply</span>
                </div>
                <p className="font-body text-[10px] text-[#b6b0a6] mt-0.5">Top: {d.topBrand}</p>
              </div>
            )
          })()}
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 mt-4 pt-4 border-t border-white/5">
            {Object.entries(regionColors).map(([region, color]) => (
              <div key={region} className="flex items-center gap-2">
                ...
                <span className="font-body text-[12px] text-[#b0a99d]">{region}</span>
                <span className="font-mono text-[10px] text-[#b6b0a6]">({Object.values(countryData).filter(d => d.region === region).length} countries)</span>
              </div>
            ))}
            <div className="flex items-center gap-2 ml-4">
              <div className="w-2 h-2 rounded-full bg-[#555]" />
              <span className="font-mono text-[10px] text-[#b6b0a6]">Dot size = specimen count</span>
            </div>
          </div>
        </div>
      </div>
